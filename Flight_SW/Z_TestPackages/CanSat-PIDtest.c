...
#define ASC_EXIT_BELOW_APOGEE 7 // mission: 20
...
#define BLADE_RELEASE_DELAY 1.0 // The number of seconds to wait after deploying from container before releasing the blades
...
#define KP_MAG 1 // The value for the proportional gain for the mag PID
#define KI_MAG 0 // The value for the integral gain for the mag PID
#define KD_MAG 0 // The value for the derivative gain for the mag PID
...
void Determine_State(int32_t *saved_initial_pressure)
{
	...
	if (ReadJumper(TEST_JUMPER) == 1) // Jumper is in place
	{
...
int main(void)
{
	...
	/*LightAll();
	int jumper1 = ReadJumper(1);
	int jumper2 = ReadJumper(2);
	int jumper3 = ReadJumper(3);*/
		...
		switch(FSW_state)
		{
			...
			case SVD:
				...
				if (!_burnsequences)
				{
					...
					PC_WriteString(buffer);
					sprintf(buffer, "setmotor: %d	", motordrive);
					PC_WriteString(buffer);
					PC_WriteString("\n");
#endif
...
#endif
					int setmotor = magcorrection + gyrocorrection;
