/*
 * This file is the main source file for the Carleton Raven Knights CanSat team of 2015.
 * This code is developped for our custom board running an MKL16Z designed by Jonathan.
 * Notable ICs on the board are: BMP180, AK8963, MPU6500, AT24CM01, DS1338, BD621, FTDI
 * 
 * Running this code the Raven Knights secured THIRD PLACE out of 62 teams!!!
 * 
 * Contributors: Erik, Maytham, Alok, Amente, Akshay, David
 * Other team members: Brad, Guiliana, Kyle, Graham, Aman
 * 
 * This code is for use, reuse and modification by any future Carleton CanSat teams using the Raven Knights name.
 * Release of this code to a wider audience using any licence must be by or cleared with Erik Johnson, Maytham Sabbagh, or Alok Deshpande.
 * 
 * FUTURE FIXES:
 * Reading the battery voltage using the voltage divider and ADC is not very accurate and could be improved
 * An external crystal could be used to improve the MIPS of the MKL16Z
 * Magnetometer has an odd problem where occasionally on powerup it reads all 1's as data
 */
...
#define LNC_EXIT_ALTITUDE 50 // mission: 50 (elevator: 10)
#define ASC_EXIT_BELOW_APOGEE 20 // mission: 20 (elevator: 5)
#define PDS_EXIT_ALTITUDE 500 // mission: 500 (elevator: 30)
...
/* BEGIN Non configuration pre-processor directives */
...
/* END Non configuration pre-processor directives */
...
// Timer interrupt with period SERVICE_T
void Interrupt_Service_TPM1()
{
	TPM1_BASE_PTR->SC |= TPM_CLR_MASK; // Clear interrupt
	
	// **** ROOM FOR IMPROVEMENT:
	// Saving 4 times a second gives only 25.6 minutes of data (21 bytes per save)
	// Either save less frequently OR (prefereably) implement a circular saving scheme in EEPROM code
	...
	case 5:
		// Sample
		_sample = true;
		// Transmit twice as fast in the debug compile
#ifdef DEBUG
		if (FSW_state != LND) // Do not transmit in landed state
			{ _transmit = true; }
		else
...
// UART connected to the FTDI chip
...
// **** ROOM FOR IMPROVEMENT:
// Use external clock to increase core clock for higher MIPS
// NOTE: this will require adjusting one of the UART timing initializations
void Basic_Init()
{
...
// Recovers the state (if no test jumper in place) if we suffer an unexpected reset in flight
...
// Simple computation of packet checksum
...
// Send the packet on the XBEE UART port
void Transmit() // 39 ms
{
...
// Save the relevant data to EEPROM
void Save() // 14 ms
{
...
// Read all sensors and place the data in the global packet
// Also returns sample_data for altitude calculations and debugging
// NOTE: this function should handle a dead second BMP (not tested)
...
// Gets the initial pressure at ground level and zeroes the altitude array
...
// Median filters the last 5 elemetns of alititudearray (filterlength variable is unused and was left in for legacy reasons)
...
// This function averages the magnetometer data (not used if numberofsamples == 1; which was the case for the actual mission)
...
// Averages the gyro data (does nothing if numberofsamples == 1; which was the case for the actual mission)
...
// Determines the descent distance in the last samplesback number of samples
// NOTE: this is used to decide when we should enter the LND state
...
// Erases the entire EEPROM chip (i.e. writes 0 to all addresses)
void EraseEEPROM()
{
...
// Writes the state just transitioned to in the RTC EEPROM
...
// Checks if data has been received on the FTDI UART port and takes any necessary action
void ServicePC()
{
...
// The program entry point on powerup or reset
int main(void)
{
	...
	int16_t Max_Altitude = 0; // Maximum altitude seen
	...
	// Perform the first sample so our first transmit is not all zeroes
	Sample();
	...
	for(;;) 
	{
		sample_data temp;
		temp = Sample(); // samples
		GPIOD_PSOR |= Mask_4; // raise pin high
		Transmit(); // transmit data
		GPIOD_PCOR |= Mask_4; // lower pin
		delay(200);
	}*/
	...
	for(;;)
	{
		...
		{
			Transmit(); // transmit data on XBEE UART port
			_transmit = false;
		}
		if (_save) // If a save duration has passed...
		{
#ifdef EEPROM_PRESENT
			Save(); // and EEPROM_PRESENT, save data to EEPROM
#endif
#ifdef RTC_PRESENT
			RTC_SaveEEPROMAddress(EEPROM_Address); // Keep track of last EEPROM address written to
#endif
			_save = false;
		}
		...
		switch(FSW_state)
		{
			...
			case TST:
...
#endif
					// Calibrate the pressure to ground level and save in RTC EEPROM
				...
				// Test to see that mag is fully functional (if error, signal this)
				// **** ROOM FOR IMPROVEMENT:
				// Fix the mag code so this never happens!
				// NOTE: it is unknown what causes this error or how to recover
#ifdef MAG_TEST
				AK8963_Data magdata = Get_Magnetometer(magcaldata);
				if (magdata.HX == 0 && magdata.HY == 0 && magdata.HZ == 0)
				{ Light(2); Light(7); }
#endif
				
				// If jumper 3 is shorted while in test state -> perform calibration routine
				if(ReadJumper(3) > 0)
				{
					// Begin calibration sequence (starts timer)
					_caltimer = 0; _calinprogress = true;
					// Signal first time entering the calibration loop
					bool firsttime = true;
					// Change LED states to indicate calibration
					if (FSW_state == TST) { Darken(4); Darken(5); }
					Light(1); Light(8);
					// Calibration loop
					while (_calinprogress)
					{
						...
						AK8963_Data magdata = Get_Magnetometer(magcaldata);
						if (firsttime) // if first time save all values
						{
							_maxHX = magdata.HX; _minHX = magdata.HX;
							_maxHY = magdata.HY; _minHY = magdata.HY;
							firsttime = false;
					...
					if (FSW_state == TST) { Light(4); Light(5); }
					Darken(1); Darken(8);
					// Save the values in RTC EEPROM (so that they persist through unexpected power loss or reset)
					RTC_SaveMagnoXCal_Min(_minHX); RTC_SaveMagnoYCal_Min(_minHY);
					RTC_SaveMagnoXCal_Max(_maxHX); RTC_SaveMagnoYCal_Max(_maxHY);
				}
			...
			case LNC:
				...
				if (_eraseeeprom)
				{
					...
					RTC_SetTime(0,0,0);
#endif
					// Change LED states to indicate exit of TST state
					Darken(4); Darken(5);
				}
...
# error need to sample something to stabilize
			...
			case LND:
				...
				// Activate buzzer and stop the motor
				if (_statetransition)
					{ ActivateLocationAssistanceDevice(); Motor_Stop(); }
